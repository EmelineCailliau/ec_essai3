view: sessions {
  derived_table: {
    ...
    sql_trigger_value: select max(created_at) from events ;;  # dès qu'il y a un nouvel événement, une session est créée à partir des champs de la table eventes
    # pour chaque session id on créé des variables en appliquant des fonctions sur les variables de la table events
    sql: SELECT
        session_id
  ...
  # on créé une "dimension" pour chaque nom de champs présent dans le SELECT de la requête
  # on créé aussi des champs calculés
...
#     timeframes: [time, date, week, month, hour_of_day, day_of_week]
  ...
  dimension: duration {
    label: "Durée (sec)"
    type: number
    sql: DATEDIFF('second', ${session_start_raw}, ${session_end_raw}) ;;  # durée = date de fin - date de début
  }
...
# création d'un booleen
  ...
  measure: all_sessions {
    view_label: "Funnel View"   # regrouper plusieurs champs dans un même groupe pour la création des look (dans le Explore)
