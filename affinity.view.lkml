# code copier du modèle donc essayer de le comprendre

view: affinity {
  derived_table: {

    datagroup_trigger: ec_essai3_default_datagroup  # dés qu'il y a des nouvelles données, on met à jour ces données sur les produits
    sql: SELECT
        product_a_id
        , product_b_id
        , joint_user_freq
